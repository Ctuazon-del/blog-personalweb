...
const BlogStore = {
  ...
  async savePostToServer(post, passcode) {
    ...
    if (!response.ok) {
      const error = new Error('Could not save to server');
  ...
  async savePost(post, passcode = '') {
    try {
      const result = await this.savePostToServer(post, passcode);
      localStorage.removeItem(this.key);
      return result;
    } catch (error) {
      if (error.status === 403) {
  ...
  async deletePost(id, passcode = '') {
    try {
      const response = await fetch(`api/posts.php?id=${encodeURIComponent(id)}`, {
        method: 'DELETE',
        headers: {
          'X-Owner-Passcode': passcode
        }
      });
      return response.ok ? await response.json() : { ok: false, unauthorized: response.status === 403 };
    } catch (error) {
